...
`downloadLowestQualityVideo(masterPlaylistUrl, fetcher, options)` 会读取主播放列表中所有的子播放列表，
选择最低码率的播放列表并下载所有分片（包含 fMP4 的 init 段），最终返回 `Blob`。如果存在独立音轨，
默认会使用 MP4Box.js 合并音频（可通过 `options.includeAudio = false` 关闭）。
...
// 若无需音频
const videoOnlyBlob = await downloadLowestQualityVideo(url, fetch, {
  includeAudio: false,
});
```
...
> 如果需要合并音频，请确保已引入 `mp4box.all.min.js`。
